...
* *Start and End Date (optional)*: Define a start and/or end timestamp if you want to restrict access to a specific time range. If no end date is given, the time of the App run (now, UTC) is used as end date, so that locations with timestamps in the future are never downloaded. If the selected time range contains no locations, the App logs an error naming the timestamps you set and stops.
...
* *Download most recent data (optional)*: Option to select download of data of a certain number of days before NOW. This option only makes sense for data with life feed into Movebank. If this option is chosen, “start date” and “end date” will be ignored; the data are downloaded from the given number of days before now until now.
* *Fast data reduction profiles (optional)*: Movebank provides fast, reduced data download options that you can select to use here. By default they are deselected. It is possible to select (a) download of the full track in resolution of 1 location per day or (b) download of the complete tracks of the last 30 days (last 30 days of the track, independently of when they ended). If tracks have more than 800 locations, the event attributes are reduced to a minimum, the track attributes remain in full. Note that selected Start and End Dates, as well as "Download most recent data", will be ignored when a profile is selected. Click "Next"
...
**Input data:** none required. If input data are provided, they must use the same *Attribute that will define and identify the tracks* as this App; otherwise the App logs *"The downloaded data cannot be combined with the input data: the track ID column differs"* and stops with an error. Select the same option in all Movebank Apps of the workflow.

**Animals and time range:** Selected Animals without data meeting the other selection criteria (sensor type, time range, outlier setting) are simply absent from the returned data set; no warning is given for them. If none of the selected Animals has data to download, the App logs *"No data are available … for the selected animals and sensors"* — naming the start and/or end timestamp you set, if any — and stops with an error, which stops the workflow. Compare the first and last location dates shown in the Study or Animal Tabs with your Start/End Date or the "Download most recent data" setting.

**Movebank credentials:** The credentials are verified with a test query before anything is downloaded. If Movebank rejects them, the App stops immediately with the error *"Your Movebank username and/or password are not valid"*. Correct the Movebank Login in your MoveApps user profile.

**Movebank unavailable:** If Movebank cannot be reached, the App retries for up to 30 minutes and then stops with an error message in the log. Requests that cannot succeed by waiting (no data for the selection, too many animals for one request) fail immediately with a message.
...
:warning: Locations with timestamps in the future are excluded from the download unless you explicitly set an end date in the future. If you do so, such obviously wrong timestamps are retained, and may cause errors or unexpected results in later steps of your workflow. They can be flagged as outliers in Movebank by a Data Manager for the study ([see instructions](https://www.movebank.org/cms/movebank-content/deployments-and-outliers#mark_outliers)) or filtered in subsequent steps of your workflow, e.g., with the [Remove Outliers App](https://github.com/movestore/RemoveOutliers). Such timestamps can represent erroneous values provided by the tag or be caused by incorrectly defining the field format when data were imported to Movebank (a Data Manager for the study can review and reimport data if needed following [these instructions](https://www.movebank.org/cms/movebank-content/upload-qc#fix_incorrectly_mapped_values)).

### Technical details
* The download uses `move2::movebank_download_study()`, the available sensor types come from `move2::movebank_retrieve(entity_type = "tag_type")`. Credentials are stored for the session with `move2::movebank_store_credentials()` (keyring backend `env`).
* Every Movebank request is wrapped in a retry loop (`retry_with_backoff()` in `RFunction.R`): retries every minute for the first 10 minutes, then every 5 minutes, up to 30 minutes in total. Errors that indicate a data, request or credential problem rather than downtime (invalid credentials, no data found, no deployed data, request URL too long) are not retried.
...
* Timestamps are handled in UTC. If no end date is set, the current time is used as end of the download window. Whether the selected time range contains data is not checked in advance (the deployment dates of the Movebank study table can be out of date); an empty result of the download itself is reported instead.
* At the end of the App log a summary line reports the number of locations downloaded from Movebank, how many were excluded as empty points, for missing coordinates and as duplicated timestamps, and how many locations are returned.
